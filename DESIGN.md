...
- Continuous rotation (~4°/sec, full rotation ~90s) via precomputed-frame cross-fading, not a live per-tick recompute — see Performance, this is load-bearing, not a style preference. Plus subtle mouse-parallax drift and a soft atmospheric rim-light glow (positioned left, matching the vertical terminator below).
...
## Performance
- **Do not drive Earth rotation with a live recompute loop.** Two earlier attempts both failed by construction: recomputing the full three-layer per-column projection often enough to look smooth (~45ms) was expensive enough to visibly lag the whole page; throttling it enough to stop lagging (~220ms) made the rotation visibly step. There is no interval that satisfies both — the recompute itself is just too expensive to do at animation-frame frequency, no matter how it's tuned. If either complaint (choppy vs. laggy) comes back, the fix is not another interval number.
- The actual fix: **precompute once, blend cheaply forever.** At load (and again after any resize that changes `earth.sphereSize`), `startFramePrecompute()` renders `FRAME_COUNT` (72, one every 5°) full three-layer composites into small cached canvases, spread one-per-`requestAnimationFrame` so it doesn't block. `drawEarth()` then cross-fades between the two frames nearest the current continuous `earth.displayLon` (`ctx.globalAlpha` at `1-t` and `t`) every animation frame — two cheap `drawImage` calls, not a recompute. `earth.displayLon` itself advances every frame by real elapsed time (`ROTATION_DEG_PER_MS * deltaMs`, not a fixed per-tick step), so motion is smooth regardless of frame rate. This mirrors what SpaceX actually does for their Mars (a pre-rendered `.mp4`, confirmed by inspecting the page directly) as closely as a canvas-generated sphere can: do the expensive part once, play the result back cheaply.
- The internal render resolution used for each cached frame is still capped well below display size (`sphereSize`, capped at 480px regardless of display diameter or DPR) and upscaled via `drawImage` — this cap is about texture crispness/memory, not the rotation cost anymore, since precomputed frames aren't regenerated during animation.
- A resize bumps `earth.buildToken` and restarts precompute from scratch; in-flight precompute steps check the token and abort if superseded, so a rapid resize can't leave two precompute runs racing to write into the same `earth.frames` array.
- Verified via `PerformanceObserver({entryTypes:['longtask']})`: zero long tasks (>50ms) over 5-6s samples both at steady state and across the initial precompute burst. Verified smoothness directly by sampling a fixed canvas pixel on the Earth's limb across 30 consecutive animation frames — color changed by only a few units every single frame with no repeated/stepped values, confirming genuine per-frame interpolation rather than the visible stepping either prior interval-based approach produced.
