...
- Continuous smooth rotation (~0.18°/45ms, full rotation ~90s) + subtle mouse-parallax drift, soft atmospheric rim-light glow (positioned left, matching the vertical terminator below). Must stay smooth/continuous, not visibly stepping — regenerate the sphere projection frequently in small increments rather than large jumps.
- Day/night terminator: a **vertical**, hard-edged line (light left, dark right — not diagonal), matching how the real spacex.com Mars phase reads. All three layers (day darkening, night-light mask, cloud fade) share one `verticalAxisGradient` helper so the boundary lines up exactly across layers.
